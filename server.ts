...
function getSyncedData() {
  ...
  const activeExpensesMapped = store.transactions.filter((t) => !t.isDeleted).map((t) => ({
    ...t,
    category: store.categories.find((c) => c.id === t.categoryId) || null,
  }));
  ...
  const activeIncomesMapped = store.incomes.filter((i) => !i.isDeleted).map((i) => ({
    ...i,
    amount: typeof i.amount === 'number' ? i.amount : parseFloat(i.amount as any),
  ...
  return {
    ...
    expenses: allExpensesSynced,
    incomes: allIncomesSynced,
  };
}
...
app.get('/api/sync', (req: Request, res: Response) => {
  ...
  const { categories, expenses, incomes } = getSyncedData();
...
app.post('/api/sync', async (req: Request, res: Response) => {
  ...
  const { categories: activeCategories, expenses: activeExpenses, incomes: activeIncomes } = getSyncedData();
